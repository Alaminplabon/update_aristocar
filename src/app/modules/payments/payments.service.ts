...
import invoice from '../invoice/invoice.models';
...
const checkout = async (payload: IPayment) => {
  ...
  if (user?.vat_type === 'Romanian') {
    const vatRate = 0.19; // 19% VAT
    const totalVat = subscription?.amount * vatRate;
    amount = totalVat + subscription?.amount;
    vat = totalVat;
...
const confirmPayment = async (query: Record<string, any>) => {
  ...
  try {
    ...
    originalOldSubscription = { ...oldSubscription?.toObject() }; // Store original oldSubscription state
    ...
    originalSubscription = { ...subscription?.toObject() }; // Store original subscription state
...
const getEarnings = async () => {
  ...
  const todayEarningsData = await Payment.aggregate([
    {
      $match: {
        isPaid: true,
        isDeleted: false,
  ...
  // Query for all data
  const allData = await Payment.aggregate([
    {
      $match: {
        isPaid: true,
      },
...
const dashboardData = async (query: Record<string, any>) => {
  ...
  const userJoinYear = query.JoinYear || moment().year();

  const startOfIncomeYear = moment().year(incomeYear).startOf('year');
  const endOfIncomeYear = moment().year(incomeYear).endOf('year');

  const startOfUserYear = moment().year(userJoinYear).startOf('year');
  const endOfUserYear = moment().year(userJoinYear).endOf('year');
  ...
  // Recent 15 users (excluding admins)
  ...
  // Recent earnings filtered by income year
  const earnings = await Payment.aggregate([
    {
      $match: {
        isPaid: true,
        createdAt: {
          $gte: startOfIncomeYear.toDate(),
          $lte: endOfIncomeYear.toDate(),
        },
      },
    },
    {
  ...
  // Total earnings for the income year
